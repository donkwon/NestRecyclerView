 ...
 */
public class NestParentRecyclerView extends RecyclerView implements NestedScrollingParent {
    private static final int SCROLL_DIRECTION_DOWN = -1;
    private static final int SCROLL_DIRECTION_UP = 1;

    private final String TAG = "NestParentRecyclerView";
    ...
    private OnScrollableChildCallback mOnScrollableChildCallback = new OnScrollableChildCallback() {
        ...
        public void onScrollableChildChangedToIdle(RecyclerView recyclerView) {
            if ((NestParentRecyclerView.this.mCurrentFlingView == recyclerView) && (NestParentRecyclerView.this.mScroller != null) && (!NestParentRecyclerView.this.mScroller.isFinished())) {
                NestParentRecyclerView.this.fling(0, (int) NestParentRecyclerView.this.mScroller.getCurrVelocity() * NestParentRecyclerView.this.mScrollDirection);
                NestParentRecyclerView.this.mScroller.abortAnimation();
            }
        }

        @Override
        public void onScrollableChildSelected(INestChildFragment paramAnonymousScrollableChild) {
            NestParentRecyclerView.this.mScrollableChild = paramAnonymousScrollableChild;
            NestParentRecyclerView.this.mScrollableView = mScrollableChild.getRecyclerView();
        }

    };

    public NestParentRecyclerView.OnScrollListener onScrollListener = new NestParentRecyclerView.OnScrollListener() {
        public void onScrollStateChanged(RecyclerView recyclerView, int newStatus) {
            if (newStatus == RecyclerView.SCROLL_STATE_IDLE) {
                getOnScrollableChildCallback().onScrollableChildChangedToIdle(recyclerView);
            }
        }
    };
    private NestedScrollingParentHelper mParentHelper;
    private int mScrollDirection = SCROLL_DIRECTION_UP;
    private INestChildFragment mScrollableChild;
    private RecyclerView mScrollableView;
    private OverScroller mScroller;
    private int mTouchSlop;

    public NestParentRecyclerView(Context paramContext) {
        this(paramContext, null);
    }

    public NestParentRecyclerView(Context paramContext, AttributeSet paramAttributeSet) {
        this(paramContext, paramAttributeSet, 0);
    }

    public NestParentRecyclerView(Context paramContext, AttributeSet paramAttributeSet, int paramInt) {
        ...
        addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                if (newState == RecyclerView.SCROLL_STATE_IDLE) {
                    if ((NestParentRecyclerView.this.mCurrentFlingView == recyclerView) && (NestParentRecyclerView.this.mScroller != null) && (!NestParentRecyclerView.this.mScroller.isFinished())) {
                        if (NestParentRecyclerView.this.isScrollableViewShown()) {
                            NestParentRecyclerView.this.debugLog("onScrollStateChanged: Parent fling " + NestParentRecyclerView.this.mScroller.getCurrVelocity());
                            NestParentRecyclerView.this.mScrollableView.fling(0, (int) NestParentRecyclerView.this.mScroller.getCurrVelocity() * NestParentRecyclerView.this.mScrollDirection);
                        }
                        NestParentRecyclerView.this.debugLog("onScrollStateChanged  abortAnimation ");
                        NestParentRecyclerView.this.mScroller.abortAnimation();
                    }
                }
            }
    ...
    private void debugLog(String paramString) {
        Log.d("NestParentRecyclerView", paramString);
    }
    ...
    public boolean isScrollableChildReachTop() {
        ...
        if ((localObject instanceof LinearLayoutManager)) {
            b = ((localView == null) || ((((LinearLayoutManager) localObject).findFirstCompletelyVisibleItemPosition() <= 0)));
        } else if (localObject instanceof StaggeredGridLayoutManager) {
            b = ((localView == null) || (((StaggeredGridLayoutManager) localObject).findFirstCompletelyVisibleItemPositions(null)[0] <= 0));
        }
        return b;
    }
    ...
    public boolean isScrollableViewShown() {
        ...
        if (this.mScrollableView != null) {
            //先移除再添加，避免重复添加造成重复回调
            this.mScrollableView.removeOnScrollListener(onScrollListener);
            this.mScrollableView.addOnScrollListener(onScrollListener);
        }
    ...
    public void onNestedPreScroll(View target, int dx, int dy, int[] consumed) {
        ...
        while ((!isScrollableViewShown()) || (this.mScrollableView.getScrollState() != 1) || (((dy <= 0) || (isReachBottom())) && ((dy >= 0) || (!isScrollableChildReachTop())))) {
            return;
        }
        consumed[1] = dy;
        scrollBy(0, dy);
    ...
    public interface OnScrollableChildCallback {
        ...
        void onScrollableChildSelected(INestChildFragment paramScrollableChild);
    }
    ...
    private void setCurrFragment(INestChildFragment fragment) {
        mOnScrollableChildCallback.onScrollableChildSelected(fragment);
    }
        ...
        addOnScrollListener(new OnScrollListener() {
            ...
            public void onScrollStateChanged(@NonNull RecyclerView recyclerView, int newState) {
                ...
                if (pagerAdapter instanceof FragmentPagerAdapter) {
                    curFragment = ((FragmentPagerAdapter) pagerAdapter).getItem(viewPager.getCurrentItem());
                } else if (pagerAdapter instanceof FragmentStatePagerAdapter) {
                    curFragment = ((FragmentStatePagerAdapter) pagerAdapter).getItem(viewPager.getCurrentItem());
                }
                if (curFragment instanceof INestChildFragment) {
                    setCurrFragment((INestChildFragment) curFragment);
                }
